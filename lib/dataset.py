...
class FoxDataset(Dataset):
    def __init__(self, data_path, cfg) -> None:
        super(FoxDataset, self).__init__()
        ...
        # load transforms
        with open(os.path.join(data_path, 'transforms.json'), 'r') as file:
            transfroms = json.load(file)
        camera_angle_x = transfroms['camera_angle_x']
        camera_angle_y = transfroms['camera_angle_y']
        frames = transfroms['frames']
        pdb.set_trace()
        random.shuffle(frames)
        self.frame_set = frames
        # load an image to get H/W
        file_path = os.path.join(self.data_path, frames[0]['file_path'])
        image = imageio.v2.imread(file_path)

        # camera intrinsics
        self.H = image.shape[0]
        self.W = image.shape[1]
        self.focal_x = 0.5 * self.W / math.tan(0.5 * camera_angle_x)
        self.focal_y = 0.5 * self.H / math.tan(0.5 * camera_angle_y)
        if self.half_size:
            self.H =  self.H // 2
            self.W =  self.W // 2
            self.focal_x =  self.focal_x / 2.0
            self.focal_y =  self.focal_y / 2.0
        
    def create_bounding_box(self):
        def create_boundary(c2w):
            boundary = []
            bd_point = [(0, 0), (0, self.H - 1), (self.W - 1, 0), (self.W - 1, self.H - 1)]
            for _, point in enumerate(bd_point):
                ray = torch.tensor(
                    [(point[0] - self.W * 0.5) / self.focal_x, -(point[1] - self.H * 0.5) / self.focal_y, -1.0], 
                    dtype=torch.float32
                )
                ray = ray @ c2w[:3, :3].T
                pts = c2w[:3, -1]
    ...
    def __getitem__(self, index):
        # load ground truth image
        file_path = os.path.join(self.data_path, self.frame_set[index]['file_path'])
        image = imageio.v2.imread(file_path)
        ...
        dirs = torch.stack([(x - self.W * 0.5) / self.focal_x, -(y - self.H * 0.5) / self.focal_y, -torch.ones_like(x)], -1)
        dirs_world = dirs @ c2w[:3, :3].T
        # dirs_world = dirs_world / torch.norm(dirs_world, dim=-1, keepdim=True)
        #! normalize了，不然有点怪。尝试了HashNeRF发现norm不norm差别不大
        pts =  c2w[:3, -1].expand(self.H, self.W, 3)
...
def make_dataset(cfg, data_path):
    if cfg['type'] == 'nerf_synthesic':
        ...
        return FoxDataset(data_path, cfg)
    else :
